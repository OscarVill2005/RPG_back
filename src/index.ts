...
io.on("connection", (socket: any) => {
  socket.on("disconnect", () => {
    if (socket.data.username) {
      ...
      io.emit(
        "user_list" + socket.data.room_code,
        Array.from(users[socket.data.room_code])
      ); // Emitir la lista de usuarios a todos
        ...
        if (gamedata.boss.health <= 0) {
          ...
          if (gamedata.game.current_player == user_emails.length - 1) {
            ...
            if (boss_action > 2) {
              ...
              console.log(
                "Vida de los jugadores:" + gamedata.player1.health_points
              );
            } else {
